...
<body>
    ...
    <div id="content">
        <h4 class="card-title">Dashboard Harian </h4>
        <hr>
        <form method="post" action="<?= base_url('owner/Dashboard'); ?>">
            <div class="row">
                <div class="col-6">
                    <input type="date" class="form-control" id="tanggal" name="tanggal" value="<?= isset($start_date) ? $start_date : date('Y-m-d'); ?>">
                </div>
        ...
        <div class="row text-sm mt-2">
            <div class="col-lg-6 ">
                <div class="card shadow-sm ">
                    <div class="card-header">
                        ...
                        <p class="card-text">Daftar Material Terlaris Hari ini</p>
                    </div>
                    <div class="card-body">
                        <?php
                        $labels_chart = [];
            ...
            <div class="col-lg-6 ">
                <div class="card shadow-sm ">
                    <div class="card-header">
                        <h5 class="card-title">Pengeluaran terbanyak</h5>
                        <p class="card-text"> Daftar Pengeluaran terbanyak Hari ini</p>
                    </div>
                    <div class="card-body">
                        <?php
        ...
        <div class="row text-sm mt-2">
            <div class="col-lg-12 ">
                <div class="card shadow-sm ">
                    <div class="card-header">
                        <h5 class="card-title">Jumlah Nominal Transaksi Harian</h5>
                        <p class="card-text"> Setiap Hari Selama Bulan Ini</p>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
    ...
    <script>
        $(document).ready(function() {
                ...
                new Chart(ctx_keluar, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data: data_trxkeluar,
                            backgroundColor: ['#0d6efd', '#dc3545', '#ffc107', '#198754', '#6610f2', '#fd7e14', '#20c997'],
                            hoverOffset: 4
                        }]
                    },
                    options: {
            ...
            if (typeof transaksiData !== 'undefined' && transaksiData.length > 0) {
                const firstDate = new Date(transaksiData[0].tanggal_trx);
                const year_total = firstDate.getFullYear();
                const month_total = firstDate.getMonth();
                const daysInMonth_total = new Date(year_total, month_total + 1, 0).getDate();
                const labels_total = Array.from({
                    length: daysInMonth_total
                }, (_, i) => i + 1);
                const nomMasuk = Array(daysInMonth_total).fill(0);
                const nomKeluar = Array(daysInMonth_total).fill(0);
                const nomMargin = Array(daysInMonth_total).fill(0);

                transaksiData.forEach(row => {
                    const day = new Date(row.tanggal_trx).getDate();
                    nomMasuk[day - 1] = row.nom_masuk;
                    nomKeluar[day - 1] = row.nom_keluar;
                    nomMargin[day - 1] = row.nom_margin;
                });
                ...
                const config = {
                    ...
                    options: {
                        ...
                        plugins: {
                            title: {
                                display: true,
                                text: `Grafik Jumlah Nominal Transaksi ${year_total}-${String(month_total+1).padStart(2,'0')}`
                            }
                        },
                        scales: {
                            x: {
