...
<body>
    ...
    <div id="content m-2">
        ...
        <div class="card m-2 shadow-sm mb-4">
            <div class="card-body">
                <h5 class="card-title mb-2">Laporan Transaksi Uang Keluar</h5>
                <p class="card-text text-muted mb-4">Menampilkan rekapitulasi Transaksi Uang Keluar berdasarkan periode waktu.</p>
                <hr class="my-3">

                <div class="mb-3 d-flex justify-content-end gap-2">
                    <a href="<?= base_url('admin/Laporan/lap_uang_keluar') ?>" class="btn btn-success" rel="noopener noreferrer">Kembali</a>
                ...
                <?php
                // print_r($trx_mtrls);
                if (!empty($trx_mtrls) && !is_null($trx_mtrls[0])) {
                    if ($kategori_id == '0') {
                ?>

                        <h6>Jenis Transaksi : Uang Keluar | Kategori : Semua jenis pengeluaran</h6>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-bordered" id="laporanPenjualanKeluar">
                                ...
                                <tbody>
                                    ...
                                    <?php if (!empty($trx_mtrls)) : ?>
                                        ...
                                        <?php foreach ($trx_mtrls as $data) : ?>
                                            <tr>
                                                ...
                                                <td><?= $data->no_account . "-" . $data->description_account; ?></td>
                                    ...
                                    <?php else : ?>
                                        <tr>
                                            <td colspan="9" class="text-center text-muted">Tidak ada data penjualan Keluar untuk periode ini.</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                                <?php if (!empty($trx_mtrls)) : ?>
                                    <tfoot class="table-light">
                                        <tr>
                                            <th colspan="2" class="text-end">GRAND TOTAL PENJUALAN</th>
                        ...
                        <h6>Jenis Transaksi : Uang Keluar | Kategori : <?= $trx_mtrls[0]->no_account . '-' . $trx_mtrls[0]->description_account; ?></h6>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-bordered" id="laporanPenjualanKeluar">
                                ...
                                <?php if (!empty($trx_mtrls)) : ?>
                                    <tfoot class="table-light">
                                        <tr>
                                            <th colspan="3" class="text-end">GRAND TOTAL PENJUALAN</th>
                                            <th class="text-end">Rp <?= number_format($grand_total, 0, ',', '.'); ?></th>
                                            <th colspan="1"></th>
                                        </tr>
                                    </tfoot>
    ...
    <script>
        ...
        function exportExcel() {
            ...
            // Contoh: window.location.href = '<?= base_url('admin/laporan/exportPenjualanKeluarExcel'); ?>' + window.location.search;
        }
        ...
        $(document).ready(function() {
            // Inisialisasi Select2 untuk filter Keluar (jika Keluar banyak)
            $('#KeluarFilter').select2({
