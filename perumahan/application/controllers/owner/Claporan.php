...

{
    ...
    public function lap_penjualan_per_perumahan()
    {
        ...
        $this->load->view('owner/lap_penjualan_per_perumahan_view', $data);
    }

    public function lap_out_per_perumahan()
    {
        $data["judul"] = "Laporan Pengeluaran Umum Per Perumahan";
        $sql = "SELECT p.id, p.nama AS nama_perum, COALESCE(SUM(t.nominal), 0) AS total_pengeluaran
                FROM tm_perumahan p
            LEFT JOIN trx_transaksi t ON t.id_perum = p.id
                AND t.tipe_transaksi = 'keluar'
                AND t.peruntukan = 'umum'
                GROUP BY p.id, p.nama
                ORDER BY p.nama";
        $data['list_perumahan'] = $this->db->query($sql)->result();
        $data['report_date'] = date('d/m/Y');
        ...
        $this->load->view('owner/lap_out_per_perumahan_view', $data);
    }
    ...
    public function lap_out_per_perumahan_detail()
    {
        ...
        if (!$id_perum) {
            redirect('owner/Claporan/lap_out_per_perumahan');
            return;
        }
        ...
        $this->load->view('owner/lap_out_per_perumahan_detail_view', $data);
    }
    ...
    public function lap_out_rumah_per_perumahan()
    {
        ...
        $this->load->view('owner/lap_out_rumah_per_perumahan_view', $data);
    }
    ...
    public function lap_out_rumah_per_perumahan_detail()
    {
        ...
        if (!$id_perum) {
            redirect('owner/Claporan/lap_out_rumah_per_perumahan');
            return;
        }
        ...
        $this->load->view('owner/lap_out_rumah_per_perumahan_detail_view', $data);
    }
    ...
    public function lap_laba_rugi_form()
    {
        ...
        $data['list_perum'] = $this->db->order_by('nama', 'ASC')->get('tm_perumahan')->result();
        $this->load->view('owner/lap_laba_rugi_form', $data);
    }
    ...
    public function lap_laba_rugi_view()
    {
        ...
        if (!$id_perum) {
            redirect('owner/Claporan/lap_laba_rugi_form');
            return;
        }
        ...
        $this->load->view('owner/lap_laba_rugi_view', $data);
    }
    ...
    public function lap_hutang_cust_view()
    {
        ...
        if (!$id_perum) {
            redirect('owner/claporan/lap_hutang_cust_form');
            return;
        }
    ...
    public function lap_list_dp()
    {
        ...
        $data['list_perum'] = $this->db->order_by('nama', 'ASC')->get('tm_perumahan')->result();
        $this->load->view('owner/lap_list_dp_form', $data);
    }
    ...
    public function lap_list_dp_view()
    {
        ...
        if (!$id_perum) {
            redirect('owner/claporan/lap_list_dp');
            return;
        }
...
where b.id_perum = ? order by b.nama_perum asc, b.norumah asc, a.id asc";
        $data['list_rumah'] = $this->db->query($sql, [$id_perum])->result();
        $this->load->view('owner/lap_list_dp_view', $data);
    }
    ...
    public function lap_hutang_cust_detail($id_hrg_penj_rumah = null)
    {
        ...
        $this->load->view('owner/lap_hutang_cust_detail', $data);
    }
    ...
    public function lap_out_total_perumah_form()
    {
        ...
        $this->load->view('owner/lap_out_total_perumah_form', $data);
    }
    ...
    public function lap_out_total_perumah_view()
    {
        ...
        $this->load->view('owner/lap_out_total_perumah_view', $data);
    }
