...
<head>
    ...
    <title>Laporan Pengeluaran Umum Per Perumahan</title>
...
<body id="page-top">
                ...
                <div class="container-fluid p-2">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body p-3">
                            <form id="formPerumDetail" action="<?= base_url('owner/Claporan/lap_out_per_perumahan_detail'); ?>" method="post">
                            ...
                            <span class="text-muted">* Klik baris untuk melihat detail pengeluaran umum perumahan.</span>
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover" id="lap_out_per_perumahan_table">
                                    ...
                                    <tbody>
                                        <?php if (!empty($list_perumahan)) : ?>
                                            ...
                                            <?php foreach ($list_perumahan as $row) : ?>
                                                <tr class="clickable-row" data-id-perum="<?= $row->id; ?>" style="cursor: pointer;">
                                                    <td><?= $no++; ?></td>
                                                    <td><?= htmlspecialchars($row->nama_perum); ?></td>
                                                    <td class="text-right">Rp <?= number_format($row->total_pengeluaran); ?></td>
                                                </tr>
                                            <?php endforeach; ?>
                                        ...
                                        <?php else : ?>
                                            <tr>
                                                <td colspan="3" class="text-center text-muted">Tidak ada data pengeluaran umum per perumahan.</td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
    ...
    <script>
        $(document).ready(function() {
            $('#lap_out_per_perumahan_table tbody').on('click', 'tr.clickable-row', function() {
                var idPerum = $(this).data('id-perum');
                if (idPerum) {
                    $('#id_perum_hidden').val(idPerum);
