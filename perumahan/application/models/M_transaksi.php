...
class M_transaksi extends CI_Model
{
    ...
    public function getAllTrxPerId($id_perum, $id_rumah, $jns_jenis)
    {
        ...
        $sql = " select * from trx_transaksi a
              where
              tipe_transaksi = 'masuk' and peruntukan = 'rumah' and  
              a.id_perum = $id_perum and a.id_rumah = $id_rumah and a.id_kateg = $jns_jenis
        ";

        return $this->db->query($sql);
        // return $this->db->query($sql);
    }
    public function get_nom_out_rumah_per_perum($id_perum)
    {
        // $this->db->where('id_perum', $id_perum);
        // $this->db->where('id_rumah', $id_rumah); // Asumsi nama kolom di DB adalah 'norumah'
        ...
        $sql = "SELECT 
                a.*,
                c.nama nama_perum
                , sum(b.nominal) total_pengeluaran
                FROM `tm_rumah` a
                LEFT JOIN trx_transaksi b on b.id_rumah = a.id  and  b.tipe_transaksi = 'keluar' and b.peruntukan='rumah'
